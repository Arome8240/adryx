    ...
    <aside className="hidden md:flex flex-col w-60 shrink-0 min-h-screen bg-[#0a0a0f] border-r border-white/8">
      ...
      <div className="px-6 py-5 border-b border-white/8">
        <Link href={URLS.adminDashboard} className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-[#a855f7] flex items-center justify-center shrink-0">
            <Shield size={16} color="#ffffff" variant="Bold" />
          </div>
          <div>
      ...
      {user && (
        <div className="px-4 py-3 border-b border-white/8 flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-[#a855f7] flex items-center justify-center text-xs font-bold text-white shrink-0">
            {getInitial(user.name)}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white truncate">{user.name}</p>
            <span className="text-[10px] text-[#a855f7] uppercase tracking-wider">Admin</span>
          </div>
        </div>
      )}
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.label}
              {active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-[#a855f7]" />}
            </Link>
          );
        })}
      </nav>
