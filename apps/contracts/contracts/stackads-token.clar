;; ============================================================
;; StackAds Token (STAD)
;; SIP-010 Fungible Token for the StackAds advertising network
;;
;; Used for:
...
(define-constant TOKEN-SYMBOL "STAD")
(define-constant TOKEN-DECIMALS u6)
(define-constant MAX-SUPPLY u1000000000000000) ;; 1 billion STAD (with 6 decimals)
...
(define-fungible-token stackads-token)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance stackads-token account)))
...
(define-read-only (get-token-uri)
  (ok (some u"https://stackads.io/token-metadata.json")))
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (asserts! (> amount u0) ERR-INVALID-AMOUNT)
    (asserts!
      (<= (+ (ft-get-supply stackads-token) amount) MAX-SUPPLY)
      ERR-MAX-SUPPLY-EXCEEDED)
    (try! (ft-mint? stackads-token amount recipient))
    (print {
      event: "mint",
      amount: amount,
      recipient: recipient,
      new-supply: (ft-get-supply stackads-token)
    })
    (ok true)))
...
(define-public (burn (amount uint))
  (begin
    ...
    (try! (ft-burn? stackads-token amount tx-sender))
    (print {
      event: "burn",
      amount: amount,
      burner: tx-sender,
